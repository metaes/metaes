...
export function evaluate(e: ASTNode,
                         env: Environment,
  ...
  if (e.type in tokens) {
      ...
      tokens[e.type](
        ...
        error => {
          if (error instanceof EmptyNodeError) {
            cerr(new LocatedError(e, new Error(`"${e.type}" tried to access non-existing descendant node.). 
    ...
    } catch (error) { // catch error in interpreters implementations
      throw error;
    }
  } else if (!e) {
...
export function evaluateArrayParametrized(array: Iterable<ASTNode>,
                                          env: Environment,
                                          config: EvaluationConfig,
                                          c: Continuation,
                                          cerr: ErrorContinuation,
...
export function evaluateArray(array: ASTNode[],
                              env: Environment,
                              config: EvaluationConfig,
                              c: Continuation,
                              cerr: ErrorContinuation) {
...
export function evaluateArrayAsync<T>(items: T[],
...
export function apply(e: ASTNode,
                      fn: Function,
                      args: any[],
                      config: EvaluationConfig,
                      thisObj?: Object): Iterable<any> {
  let result;
  try {
