...
export function CallExpression(e: CallExpression,
  ...
  evaluateArray(e.arguments, env, config, args => {
      let calleeNode = e.callee;

      switch (calleeNode.type) {
        case 'MemberExpression':
          evaluate(calleeNode.object, env, config, object => {
              evaluate(calleeNode, env, config, property => {
        ...
        case 'CallExpression':
          evaluate(calleeNode, env, config,
            callee => {
              try {
        ...
        case 'ArrowFunctionExpression':
          evaluate(calleeNode, env, config, callee => {
              try {
                getValue(
                  env,
                  'this',
...
export function MemberExpression(e: MemberExpression,
                                 env,
                                 config,
                                 c,
                                 cerr) {
  evaluate(e.object, env, config, object => {
      if (e.computed) {
        evaluate(e.property, env, config, property => {
            c(object[property]);
          },
          cerr);
      } else {
        switch (e.property.type) {
          case 'Identifier':
            if (e.computed) {
              evaluate(e.property, env, config, value => {
                  c(object[value]);
                },
                cerr);
            } else {
...
export function AssignmentExpression(e: AssignmentExpression, env, config, c, cerr) {
  evaluate(e.right, env, config, right => {
      switch (e.left.type) {
        case 'MemberExpression':
          let left = e.left;
          evaluate(e.left.object, env, config, object => {
              ...
              if (left.property.type === 'Identifier') {
                ...
                evaluate(left.property, env, config, propertyName => {
                    doAssign(object, propertyName, right);
                  },
                  cerr);
              }
...
export function ObjectExpression(e: ObjectExpression, env, config, c, cerr) {
  let object = {};
  evaluateArray(e.properties, env, config, properties => {
...
export function BinaryExpression(e: BinaryExpression, env, config, c, cerr) {
  evaluate(e.left, env, config, left => {
      evaluate(e.right, env, config, right => {
          switch (e.operator) {
            case "+":
              c(left + right);
  ...
  evaluateArray(e.arguments, env, config, args => {
      ...
      switch (calleeNode.type) {
        case "MemberExpression":
          ...
          break;
        case 'Identifier':
          getValue(env, calleeNode.name, callee => {
              if (typeof callee !== "function") {
                cerr(new LocatedError(e, new TypeError(typeof callee + " is not a function")));
                // TODO: use if/else
                return;
...
export function SequenceExpression(e: SequenceExpression, env, config, c, cerr) {
  evaluateArray(e.expressions, env, config, results => {
      if (results.length) {
...
export function UpdateExpression(e: UpdateExpression, env: Environment, _config, c, cerr) {
  switch (e.argument.type) {
    case 'Identifier':
      getReference(env, e.argument.name,
        reference => {
...
export function UnaryExpression(e: UnaryExpression, env: Environment, config, c, cerr) {
  evaluate(e.argument, env, config, argument => {
      switch (e.operator) {
        case "typeof":
          c(typeof argument);
          break;
