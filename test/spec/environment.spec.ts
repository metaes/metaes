...
describe("Environment", () => {
  it("should merge two or more environments", async () => {
    ...
    assert.equal(await evalAsPromise(ctx, "a"), 1);
    assert.equal(await evalAsPromise(ctx, "a", { values: { a: 2 } }), 2);

    assert.deepEqual(await evalAsPromise(ctx, "[a,b]", { values: { b: 1 } }), [1, 1]);
    ...
    assert.deepEqual(await evalAsPromise(ctx, "[a,b,c]", env4), [1, 2, 3]);
  });
});
